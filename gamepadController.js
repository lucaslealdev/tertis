  ...
  var GamepadController = (function () {
    function GamepadController(gamepadIndex) {
      ...
      this.buttonEventListeners = Array.from({length: 17}, function () {
        return { onPress: null, onRelease: null, onChange: null, onHold: null };
      });
      this.buttonEventRepeaters = Array.from({length: 17}, function () {
        return null;
      });
    ...
    GamepadController.prototype.onGamepadConnected = function () {
      ...
      this.previousButtonStates = Array.from({length: 99}, () => ({pressed: false, value: 0}));
    ...
    GamepadController.prototype.emitButtonHold = function (index, state) {
      this.emitButton('onHold', index, state);
    };
      ...
      gp.buttons
        ...
        .forEach(function (button, i) {
          ...
          if (button.value > 0 && button.pressed !== this.previousButtonStates[i].pressed) {
            ...
            if (this.buttonEventRepeaters[i]) clearInterval(this.buttonEventRepeaters[i]);
            let delay = 3;
            this.buttonEventRepeaters[i] = setInterval(() => {
              delay--;
              if (delay > 0) return;
              this.emitButtonHold(i, { previous: this.previousButtonStates[i], current: button });
            }, 100);
            this.previousButtonStates[i] = { pressed: true, value: button.value };
            return;
          }
    ...
    GamepadController.prototype.onButtonPress = function (index, cb) {
      if (typeof index === 'string') index = this.btn[index] || 0;
    ...
    GamepadController.prototype.onButtonHold = function (index, cb) {
      if (typeof index === 'string') index = this.btn[index] || 0;
      this.buttonEventListeners[index].onHold = cb;
    };
    GamepadController.prototype.removeOnButtonHold = function (index) {
      if (typeof index === 'string') index = this.btn[index] || 0;
      this.buttonEventListeners[index].onHold = null;
    };
